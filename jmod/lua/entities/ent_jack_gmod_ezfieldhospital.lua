	...
	function ENT:TryHeal()
		...
		if (Injury > 0) or (Rads > 0) or gassed or contaminated then
			if Bleed > 0 then
				...
				self.Patient:ChatPrint( "stopping bleeding")
				self:HealEffect()
			elseif Rads > 0 or contaminated then
				...
				self.Patient:ChatPrint( "decontaminating")
			elseif gassed then
				...
				self:HealEffect("hl1/ambience/steamburst1.wav", true)
				self.Patient:ChatPrint( "curing poisoning")
			else
				if Infection > 1 then
					self.Patient.EZvirus.Severity = math.Clamp(Infection - self.HealEfficiency * JMod.Config.MedBayHealMult * 3, 1, 9e9)
					self.Patient:ChatPrint( "boosting immune system")
				else
					self.Patient:ChatPrint( "repairing damage")
				end
